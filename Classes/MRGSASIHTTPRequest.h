...
@class MRGSASIDataDecompressor;

extern NSString *MRGSASIHTTPRequestVersion;
...
typedef enum _MRGSASIAuthenticationState {
	MRGSASINoAuthenticationNeededYet = 0,
	MRGSASIHTTPAuthenticationNeeded = 1,
	MRGSASIProxyAuthenticationNeeded = 2
} MRGSASIAuthenticationState;

typedef enum _MRGSASINetworkErrorType {
    MRGSASIConnectionFailureErrorType = 1,
    MRGSASIRequestTimedOutErrorType = 2,
    MRGSASIAuthenticationErrorType = 3,
    MRGSASIRequestCancelledErrorType = 4,
    MRGSASIUnableToCreateRequestErrorType = 5,
    MRGSASIInternalErrorWhileBuildingRequestType  = 6,
    MRGSASIInternalErrorWhileApplyingCredentialsType  = 7,
	MRGSASIFileManagementError = 8,
	MRGSASITooMuchRedirectionErrorType = 9,
	MRGSASIUnhandledExceptionError = 10,
	MRGSASICompressionError = 11
	
} MRGSASINetworkErrorType;


// The error domain that all errors generated by MRGSASIHTTPRequest use
...
extern unsigned long const MRGSASIWWANBandwidthThrottleAmount;

#if NS_BLOCKS_AVAILABLE
typedef void (^MRGSASIBasicBlock)(void);
typedef void (^MRGSASIHeadersBlock)(NSDictionary *responseHeaders);
typedef void (^MRGSASISizeBlock)(long long size);
typedef void (^MRGSASIProgressBlock)(unsigned long long size, unsigned long long total);
typedef void (^MRGSASIDataBlock)(NSData *data);
#endif

@interface MRGSASIHTTPRequest : NSOperation <NSCopying> {
	...
	// The delegate - will be notified of various changes in state via the MRGSASIHTTPRequestDelegate protocol
	id <MRGSASIHTTPRequestDelegate> delegate;
	
	// Another delegate that is also notified of request status changes and progress updates
	// Generally, you won't use this directly, but MRGSASINetworkQueue sets itself as the queue so it can proxy updates to its own delegates
	// NOTE: WILL BE RETAINED BY THE REQUEST
	id <MRGSASIHTTPRequestDelegate, MRGSASIProgressDelegate> queue;
	...
	// Automatically set to true in MRGSASIFormDataRequests when using setFile:forKey:
	...
	// Set to true when MRGSASIHTTPRequest automatically created a temporary file containing the request body (when true, the file at postBodyFilePath will be deleted at the end of the request)
	...
	// If error code is = MRGSASIConnectionFailureErrorType (1, Connection failure occurred) - inspect [[error userInfo] objectForKey:NSUnderlyingErrorKey] for more information
	NSError *error;
	...
	id <MRGSASIProgressDelegate> uploadProgressDelegate;
	...
	id <MRGSASIProgressDelegate> downloadProgressDelegate;
	...
	// If you are using Basic authentication and want to force MRGSASIHTTPRequest to send an authorization header without waiting for a 401, you must set this to (NSString *)kCFHTTPAuthenticationSchemeBasic
	NSString *authenticationScheme;
	
	// Realm for authentication when credentials are required
	NSString *authenticationRealm;
	
	// When YES, MRGSASIHTTPRequest will present a dialog allowing users to enter credentials when no-matching credentials were found for a server that requires authentication
	...
	// When YES, MRGSASIHTTPRequest will present a dialog allowing users to enter credentials when no-matching credentials were found for a proxy server that requires authentication
	...
	// If you set this and implement the method in your delegate, you must handle the data yourself - MRGSASIHTTPRequest will not populate responseData or write the data to downloadDestinationPath
	...
	MRGSASIHTTPRequest *mainRequest;
	
	// When NO, this request will only update the progress indicator when it completes
	// When YES, this request will update the progress indicator according to how much data it has received so far
	// The default for requests is YES
	// Also see the comments in MRGSASINetworkQueue.h
	...
	// Tells MRGSASIHTTPRequest not to delete partial downloads, and allows it to use an existing file to resume a download. Defaults to NO.
	...
	// Details on the proxy to use - you could set these yourself, but it's probably best to let MRGSASIHTTPRequest detect the system proxy settings
	NSString *proxyHost;
	int proxyPort;
	
	// MRGSASIHTTPRequest will assume kCFProxyTypeHTTP if the proxy type could not be automatically determined
	...
	// See MRGSASIAuthenticationState values above. 0 == default == No authentication needed yet
	MRGSASIAuthenticationState authenticationNeeded;
	
	// When YES, MRGSASIHTTPRequests will present credentials from the session store for requests to the same server before being asked for them
	...
	// Temporarily set to YES when a closed connection forces a retry (internally, this stops MRGSASIHTTPRequest cleaning up a temporary post body)
	...
	// Will be MRGSASIHTTPRequestRunLoopMode for synchronous requests, NSDefaultRunLoopMode for all other requests
	NSString *runLoopMode;
	
	// This timer checks up on the request every 0.25 seconds, and updates progress
	NSTimer *statusTimer;
	
	// The download cache that will be used for this request (use [MRGSASIHTTPRequest setDefaultCache:cache] to configure a default cache
	id <MRGSASICacheDelegate> downloadCache;
	
	// The cache policy that will be used for this request - See MRGSASICacheDelegate.h for possible values
	MRGSASICachePolicy cachePolicy;
	
	// The cache storage policy that will be used for this request - See MRGSASICacheDelegate.h for possible values
	MRGSASICacheStoragePolicy cacheStoragePolicy;
	...
	MRGSASIDataDecompressor *dataDecompressor;
	...
	//
	// Setting this to NO may be especially useful for users using MRGSASIHTTPRequest in conjunction with a streaming parser, as it will allow partial gzipped responses to be inflated and passed on to the parser while the request is still running
	...
	MRGSASIHTTPRequest *PACFileRequest;
	...
	MRGSASIBasicBlock startedBlock;

	//block to execute when headers are received
	MRGSASIHeadersBlock headersReceivedBlock;

	//block to execute when request completes successfully
	MRGSASIBasicBlock completionBlock;

	//block to execute when request fails
	MRGSASIBasicBlock failureBlock;

	//block for when bytes are received
	MRGSASIProgressBlock bytesReceivedBlock;

	//block for when bytes are sent
	MRGSASIProgressBlock bytesSentBlock;

	//block for when download size is incremented
	MRGSASISizeBlock downloadSizeIncrementedBlock;

	//block for when upload size is incremented
	MRGSASISizeBlock uploadSizeIncrementedBlock;

	//block for handling raw bytes received
	MRGSASIDataBlock dataReceivedBlock;

	//block for handling authentication
	MRGSASIBasicBlock authenticationNeededBlock;

	//block for handling proxy authentication
	MRGSASIBasicBlock proxyAuthenticationNeededBlock;
	
    //block for handling redirections, if you want to
    MRGSASIBasicBlock requestRedirectedBlock;
	#endif
}
...
// Used internally to record bandwidth use, and by MRGSASIInputStreams when uploading. It's probably best if you don't mess with this.
+ (void)incrementBandwidthUsedInLastSecond:(unsigned long)bytes;

// On iPhone, MRGSASIHTTPRequest can automatically turn throttling on and off as the connection type changes between WWAN and WiFi
...
@property (assign) MRGSASIHTTPRequest *mainRequest;
...
@property (assign, readonly) MRGSASIAuthenticationState authenticationNeeded;
...
@property (assign) id <MRGSASICacheDelegate> downloadCache;
@property (assign) MRGSASICachePolicy cachePolicy;
@property (assign) MRGSASICacheStoragePolicy cacheStoragePolicy;
...
#endif
@property (retain) MRGSASIDataDecompressor *dataDecompressor;
