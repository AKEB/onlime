...
#if TARGET_OS_IPHONE
#import "MRGSReachability.h"
#import "MRGSASIAuthenticationDialog.h"
#import <MobileCoreServices/MobileCoreServices.h>
#else
#import <SystemConfiguration/SystemConfiguration.h>
#endif
#import "MRGSASIInputStream.h"
#import "MRGSASIDataDecompressor.h"
#import "MRGSASIDataCompressor.h"

// Automatically set on build
NSString *MRGSASIHTTPRequestVersion = @"v1.8.1-61 2011-09-19";

static NSString *defaultUserAgent = nil;

NSString* const NetworkRequestErrorDomain = @"MRGSASIHTTPRequestErrorDomain";

static NSString *MRGSASIHTTPRequestRunLoopMode = @"MRGSASIHTTPRequestRunLoopMode";
...
static void ReadStreamClientCallBack(CFReadStreamRef readStream, CFStreamEventType type, void *clientCallBackInfo) {
    [((MRGSASIHTTPRequest*)clientCallBackInfo) handleNetworkEvent: type];
}
...
static NSError *MRGSASIRequestCancelledError;
static NSError *MRGSASIRequestTimedOutError;
static NSError *MRGSASIAuthenticationError;
static NSError *MRGSASIUnableToCreateRequestError;
static NSError *MRGSASITooMuchRedirectionError;
...
unsigned long const MRGSASIWWANBandwidthThrottleAmount = 14800;
...
// When using MRGSASIAuthenticationDialogs, it also ensures only one dialog is shown at once
...
static id <MRGSASICacheDelegate> defaultCache = nil;

// Used for tracking when requests are using the network
static unsigned int runningRequestCount = 0;

// You can use [MRGSASIHTTPRequest setShouldUpdateNetworkActivityIndicator:NO] if you want to manage it yourself
...
- (void)fetchPACFile;
- (void)finishedDownloadingPACFile:(MRGSASIHTTPRequest *)theRequest;
...
- (void)performBlockOnMainThread:(MRGSASIBasicBlock)block;
- (void)releaseBlocksOnMainThread;
+ (void)releaseBlocks:(NSArray *)blocks;
- (void)callBlock:(MRGSASIBasicBlock)block;
#endif
...
@property (assign) MRGSASIAuthenticationState authenticationNeeded;
...
@property (retain, nonatomic) MRGSASIHTTPRequest *PACFileRequest;
...
@implementation MRGSASIHTTPRequest

#pragma mark init / dealloc

+ (void)initialize
{
	if (self == [MRGSASIHTTPRequest class]) {
		...
		MRGSASIRequestTimedOutError = [[NSError alloc] initWithDomain:NetworkRequestErrorDomain code:MRGSASIRequestTimedOutErrorType userInfo:[NSDictionary dictionaryWithObjectsAndKeys:@"The request timed out",NSLocalizedDescriptionKey,nil]];  
		MRGSASIAuthenticationError = [[NSError alloc] initWithDomain:NetworkRequestErrorDomain code:MRGSASIAuthenticationErrorType userInfo:[NSDictionary dictionaryWithObjectsAndKeys:@"Authentication needed",NSLocalizedDescriptionKey,nil]];
		MRGSASIRequestCancelledError = [[NSError alloc] initWithDomain:NetworkRequestErrorDomain code:MRGSASIRequestCancelledErrorType userInfo:[NSDictionary dictionaryWithObjectsAndKeys:@"The request was cancelled",NSLocalizedDescriptionKey,nil]];
		MRGSASIUnableToCreateRequestError = [[NSError alloc] initWithDomain:NetworkRequestErrorDomain code:MRGSASIUnableToCreateRequestErrorType userInfo:[NSDictionary dictionaryWithObjectsAndKeys:@"Unable to create request (bad url?)",NSLocalizedDescriptionKey,nil]];
		MRGSASITooMuchRedirectionError = [[NSError alloc] initWithDomain:NetworkRequestErrorDomain code:MRGSASITooMuchRedirectionErrorType userInfo:[NSDictionary dictionaryWithObjectsAndKeys:@"The request failed because it redirected too many times",NSLocalizedDescriptionKey,nil]];
...
- (id)initWithURL:(NSURL *)newURL
{
	...
	[self setTimeOutSeconds:[MRGSASIHTTPRequest defaultTimeOutSeconds]];
...
+ (id)requestWithURL:(NSURL *)newURL usingCache:(id <MRGSASICacheDelegate>)cache
{
	return [self requestWithURL:newURL usingCache:cache andCachePolicy:MRGSASIUseDefaultCachePolicy];
}

+ (id)requestWithURL:(NSURL *)newURL usingCache:(id <MRGSASICacheDelegate>)cache andCachePolicy:(MRGSASICachePolicy)policy
{
	MRGSASIHTTPRequest *request = [[[self alloc] initWithURL:newURL] autorelease];
...
- (void)dealloc
{
	[self setAuthenticationNeeded:MRGSASINoAuthenticationNeededYet];
...
- (void)buildPostBody
{
	...
	if ([self postBodyFilePath]) {
		...
		if ([self shouldCompressRequestBody]) {
			if (![self compressedPostBodyFilePath]) {
				...
				NSError *err = nil;
				if (![MRGSASIDataCompressor compressDataFromFile:[self postBodyFilePath] toFile:[self compressedPostBodyFilePath] error:&err]) {
		...
		if (err) {
			[self failWithError:[NSError errorWithDomain:NetworkRequestErrorDomain code:MRGSASIFileManagementError userInfo:[NSDictionary dictionaryWithObjectsAndKeys:[NSString stringWithFormat:@"Failed to get attributes for file at path '%@'",path],NSLocalizedDescriptionKey,error,NSUnderlyingErrorKey,nil]]];
			return;
		}
	...
	// Otherwise, we have an in-memory request body
		...
		if ([self shouldCompressRequestBody]) {
			NSError *err = nil;
			NSData *compressedBody = [MRGSASIDataCompressor compressData:[self postBody] error:&err];
			if (err) {
				[self failWithError:err];
				return;
			}
...
- (void)cancelOnRequestThread
{
	...
	MRGSASI_DEBUG_LOG(@"[STATUS] Request cancelled: %@",self);
	#endif
	...
	[self failWithError:MRGSASIRequestCancelledError];
...
{	
	if ([self isResponseCompressed] && [self shouldWaitToInflateCompressedResponses]) {
		return [MRGSASIDataDecompressor uncompressData:[self rawResponseData] error:NULL];
	} else {
...
- (void)startSynchronous
{
	...
	MRGSASI_DEBUG_LOG(@"[STATUS] Starting synchronous request %@",self);
#endif
	[self setSynchronous:YES];
	[self setRunLoopMode:MRGSASIHTTPRequestRunLoopMode];
...
{
#if DEBUG_REQUEST_STATUS || DEBUG_THROTTLING
	MRGSASI_DEBUG_LOG(@"[STATUS] Starting asynchronous request %@",self);
#endif
...
- (void)main
{
	@try {
		...
		if ([MRGSASIHTTPRequest isMultitaskingSupported] && [self shouldContinueWhenAppEntersBackground]) {
		...
		*/
		...
		// A HEAD request generated by an MRGSASINetworkQueue may have set the error already. If so, we should not proceed.
		if ([self error]) {
			[self setComplete:YES];
			[self markAsFinished];
			return;		
		...
		if (![self url]) {
			[self failWithError:MRGSASIUnableToCreateRequestError];
			return;		
		}
		...
		//If this is a HEAD request generated by an MRGSASINetworkQueue, we need to let the main request generate its headers first so we can use them
		if ([self mainRequest]) {
			[[self mainRequest] buildRequestHeaders];
		}
		
		// Even if this is a HEAD request with a mainRequest, we still need to call to give subclasses a chance to add their own to HEAD requests (MRGSASIS3Request does this)
		...
		if ([self downloadCache]) {
			...
			if ([self cachePolicy] & (MRGSASIAskServerIfModifiedWhenStaleCachePolicy|MRGSASIAskServerIfModifiedCachePolicy)) {
		...
		NSError *underlyingError = [NSError errorWithDomain:NetworkRequestErrorDomain code:MRGSASIUnhandledExceptionError userInfo:[exception userInfo]];
		[self failWithError:[NSError errorWithDomain:NetworkRequestErrorDomain code:MRGSASIUnhandledExceptionError userInfo:[NSDictionary dictionaryWithObjectsAndKeys:[exception name],NSLocalizedDescriptionKey,[exception reason],NSLocalizedFailureReasonErrorKey,underlyingError,NSUnderlyingErrorKey,nil]]];
...
- (void)applyAuthorizationHeader
{
	...
	if (![self shouldPresentCredentialsBeforeChallenge]) {
		...
		MRGSASI_DEBUG_LOG(@"[AUTH] Request %@ will not send credentials to the server until it asks for them",self);
		#endif
		return;
	}
	...
	if (![[self requestHeaders] objectForKey:@"Authorization"]) {
		...
		if ([self username] && [self password] && [[self authenticationScheme] isEqualToString:(NSString *)kCFHTTPAuthenticationSchemeBasic]) {
			...
			#if DEBUG_HTTP_AUTHENTICATION
			MRGSASI_DEBUG_LOG(@"[AUTH] Request %@ has a username and password set, and was manually configured to use BMRGSASIC. Will send credentials without waiting for an authentication challenge",self);	
			#endif
				...
				if (credentials) {
					...
					if ([credentials objectForKey:@"Authentication"]) {
						...
						if (CFHTTPMessageApplyCredentialDictionary(request, (CFHTTPAuthenticationRef)[credentials objectForKey:@"Authentication"], (CFDictionaryRef)[credentials objectForKey:@"Credentials"], NULL)) {
							...
							#if DEBUG_HTTP_AUTHENTICATION
							MRGSASI_DEBUG_LOG(@"[AUTH] Request %@ found cached credentials (%@), will reuse without waiting for an authentication challenge",self,[credentials objectForKey:@"AuthenticationScheme"]);
							#endif
						} else {
							[[self class] removeAuthenticationCredentialsFromSessionStore:[credentials objectForKey:@"Credentials"]];
							#if DEBUG_HTTP_AUTHENTICATION
							MRGSASI_DEBUG_LOG(@"[AUTH] Failed to apply cached credentials to request %@. These will be removed from the session store, and this request will wait for an authentication challenge",self);
							#endif
						}
					...
					// When this happens, we'll need to create the Authorization header ourselves
						...
						#if DEBUG_HTTP_AUTHENTICATION
						MRGSASI_DEBUG_LOG(@"[AUTH] Request %@ found cached BMRGSASIC credentials from a previous request. Will send credentials without waiting for an authentication challenge",self);
						#endif
					}
				}
			}
...
- (void)buildRequestHeaders
{
	...
	if (![[self requestHeaders] objectForKey:@"User-Agent"]) {
		...
		if (!tempUserAgentString) {
			tempUserAgentString = [MRGSASIHTTPRequest defaultUserAgentString];
		}
		if (tempUserAgentString) {
...
- (void)updatePartialDownloadSize
{
	...
	if ([self allowResumeForFileDownloads] && [self downloadDestinationPath] && [self temporaryFileDownloadPath] && [fileManager fileExistsAtPath:[self temporaryFileDownloadPath]]) {
		...
		if (err) {
			[self failWithError:[NSError errorWithDomain:NetworkRequestErrorDomain code:MRGSASIFileManagementError userInfo:[NSDictionary dictionaryWithObjectsAndKeys:[NSString stringWithFormat:@"Failed to get attributes for file at path '%@'",[self temporaryFileDownloadPath]],NSLocalizedDescriptionKey,error,NSUnderlyingErrorKey,nil]]];
			return;
		}
	}
}
...
- (void)startRequest
{
	...
	if ([self shouldStreamPostDataFromDisk] && [self postBodyFilePath] && [fileManager fileExistsAtPath:[self postBodyFilePath]]) {
		...
		if ([self compressedPostBodyFilePath] && [fileManager fileExistsAtPath:[self compressedPostBodyFilePath]]) {
			[self setPostBodyReadStream:[MRGSASIInputStream inputStreamWithFileAtPath:[self compressedPostBodyFilePath] request:self]];
		} else {
			[self setPostBodyReadStream:[MRGSASIInputStream inputStreamWithFileAtPath:[self postBodyFilePath] request:self]];
		}
		...
		if ([self postBody] && [[self postBody] length] > 0) {
			if ([self shouldCompressRequestBody] && [self compressedPostBody]) {
				[self setPostBodyReadStream:[MRGSASIInputStream inputStreamWithData:[self compressedPostBody] request:self]];
			} else if ([self postBody]) {
				[self setPostBodyReadStream:[MRGSASIInputStream inputStreamWithData:[self postBody] request:self]];
			}
			[self setReadStream:[NSMakeCollectable(CFReadStreamCreateForStreamedHTTPRequest(kCFAllocatorDefault, request,(CFReadStreamRef)[self postBodyReadStream])) autorelease]];
	...
	if (![self readStream]) {
		[self failWithError:[NSError errorWithDomain:NetworkRequestErrorDomain code:MRGSASIInternalErrorWhileBuildingRequestType userInfo:[NSDictionary dictionaryWithObjectsAndKeys:@"Unable to create read stream",NSLocalizedDescriptionKey,nil]]];
        return;
    }
	...
	[MRGSASIHTTPRequest expirePersistentConnections];
	...
	if ([self shouldAttemptPersistentConnection]) {
		...
		if ([self connectionInfo]) {
			...
			// Check if we should have expired this connection
				...
				MRGSASI_DEBUG_LOG(@"[CONNECTION] Not re-using connection #%i because it has expired",[[[self connectionInfo] objectForKey:@"id"] intValue]);
				#endif
				...
				#if DEBUG_PERSISTENT_CONNECTIONS
                MRGSASI_DEBUG_LOG(@"%@ - Not re-using connection #%i for request #%i because it is already used by request #%i",self,[[[self connectionInfo] objectForKey:@"id"] intValue],[[self requestID] intValue],[[[self connectionInfo] objectForKey:@"request"] intValue]);
				#endif
                [self setConnectionInfo:nil];
            }
		}
		...
		#if DEBUG_PERSISTENT_CONNECTIONS
		MRGSASI_DEBUG_LOG(@"[CONNECTION] Request #%@ will use connection #%i",[self requestID],[[[self connectionInfo] objectForKey:@"id"] intValue]);
		#endif
		...
		CFReadStreamSetProperty((CFReadStreamRef)[self readStream], CFSTR("MRGSASIStreamID"), [[self connectionInfo] objectForKey:@"id"]);
	
	} else {
		#if DEBUG_PERSISTENT_CONNECTIONS
		MRGSASI_DEBUG_LOG(@"[CONNECTION] Request %@ will not use a persistent connection",self);
		#endif
	}
   ...
   // Start the HTTP connection
	...
	if (!streamSuccessfullyOpened) {
		...
		[self failWithError:[NSError errorWithDomain:NetworkRequestErrorDomain code:MRGSASIInternalErrorWhileBuildingRequestType userInfo:[NSDictionary dictionaryWithObjectsAndKeys:@"Unable to start HTTP connection",NSLocalizedDescriptionKey,nil]]];
		return;	
	}
	...
	if (![self mainRequest]) {
		if ([self shouldResetUploadProgress]) {
			...
			[MRGSASIHTTPRequest updateProgressIndicator:&uploadProgressDelegate withProgress:0 ofTotal:1];
		}
		if ([self shouldResetDownloadProgress] && ![self partialDownloadSize]) {
			[MRGSASIHTTPRequest updateProgressIndicator:&downloadProgressDelegate withProgress:0 ofTotal:1];
		}
	}	
...
- (void)performRedirect
{
	...
	if ([self redirectCount] > RedirectionLimit) {
		...
		[self failWithError:MRGSASITooMuchRedirectionError];
		[self setComplete:YES];
	} else {
...
- (void)checkRequestStatus
{
	...
	if ([self shouldTimeOut]) {			
		...
		[self failWithError:MRGSASIRequestTimedOutError];
		[self cancelLoad];
		[self setComplete:YES];
		[[self cancelledLock] unlock];
		return;
	...
	if ([self readStream]) {
		...
		if ([self postLength]) {
			...
			if (totalBytesSent > lastBytesSent) {
				...
				[self setLastActivityTime:[NSDate date]];
				[MRGSASIHTTPRequest incrementBandwidthUsedInLastSecond:(unsigned long)(totalBytesSent-lastBytesSent)];		
						
				#if DEBUG_REQUEST_STATUS
				if ([self totalBytesSent] == [self postLength]) {
					MRGSASI_DEBUG_LOG(@"[STATUS] Request %@ finished uploading data",self);
				}
				#endif
			}
		}
...
// Used by MRGSASINetworkQueue to create a HEAD request appropriate for this request with the same headers (though you can use it yourself)
- (MRGSASIHTTPRequest *)HEADRequest
{
	MRGSASIHTTPRequest *headRequest = [[self class] requestWithURL:[self url]];
...
- (void)setUploadProgressDelegate:(id)newDelegate
{
	...
	double max = 1.0;
	[MRGSASIHTTPRequest performSelector:@selector(setMaxValue:) onTarget:&uploadProgressDelegate withObject:nil amount:&max callerToRetain:nil];
	#endif
	[[self cancelledLock] unlock];
}
...
- (void)setDownloadProgressDelegate:(id)newDelegate
{
	...
	double max = 1.0;
	[MRGSASIHTTPRequest performSelector:@selector(setMaxValue:) onTarget:&downloadProgressDelegate withObject:nil amount:&max callerToRetain:nil];	
	#endif
	[[self cancelledLock] unlock];
}
...
- (void)updateDownloadProgress
{
	...
	[MRGSASIHTTPRequest performSelector:@selector(request:didReceiveBytes:) onTarget:&queue withObject:self amount:&value callerToRetain:self];
	[MRGSASIHTTPRequest performSelector:@selector(request:didReceiveBytes:) onTarget:&downloadProgressDelegate withObject:self amount:&value callerToRetain:self];

	[MRGSASIHTTPRequest updateProgressIndicator:&downloadProgressDelegate withProgress:[self totalBytesRead]+[self partialDownloadSize] ofTotal:[self contentLength]+[self partialDownloadSize]];
...
- (void)updateUploadProgress
{
	...
	[MRGSASIHTTPRequest performSelector:@selector(request:didSendBytes:) onTarget:&queue withObject:self amount:&value callerToRetain:self];
	[MRGSASIHTTPRequest performSelector:@selector(request:didSendBytes:) onTarget:&uploadProgressDelegate withObject:self amount:&value callerToRetain:self];
	[MRGSASIHTTPRequest updateProgressIndicator:&uploadProgressDelegate withProgress:[self totalBytesSent]-[self uploadBufferSize] ofTotal:[self postLength]-[self uploadBufferSize]];
...
{
	[MRGSASIHTTPRequest performSelector:@selector(request:incrementDownloadSizeBy:) onTarget:&queue withObject:self amount:&length callerToRetain:self];
	[MRGSASIHTTPRequest performSelector:@selector(request:incrementDownloadSizeBy:) onTarget:&downloadProgressDelegate withObject:self amount:&length callerToRetain:self];
...
{
	[MRGSASIHTTPRequest performSelector:@selector(request:incrementUploadSizeBy:) onTarget:&queue withObject:self amount:&length callerToRetain:self];
	[MRGSASIHTTPRequest performSelector:@selector(request:incrementUploadSizeBy:) onTarget:&uploadProgressDelegate withObject:self amount:&length callerToRetain:self];
...
-(void)removeUploadProgressSoFar
{
	...
	[MRGSASIHTTPRequest performSelector:@selector(request:didSendBytes:) onTarget:&queue withObject:self amount:&progressToRemove callerToRetain:self];
	[MRGSASIHTTPRequest performSelector:@selector(request:didSendBytes:) onTarget:&uploadProgressDelegate withObject:self amount:&progressToRemove callerToRetain:self];
	[MRGSASIHTTPRequest updateProgressIndicator:&uploadProgressDelegate withProgress:0 ofTotal:[self postLength]];
...
+ (void)performSelector:(SEL)selector onTarget:(id *)target withObject:(id)object amount:(void *)amount callerToRetain:(id)callerToRetain
{
	if ([*target respondsToSelector:selector]) {
        ...
        NSMethodSignature *cbSignature = [MRGSASIHTTPRequest methodSignatureForSelector:callback];
...
+ (void)updateProgressIndicator:(id *)indicator withProgress:(unsigned long long)progress ofTotal:(unsigned long long)total
{
	...
	[progressLock lock];
	[MRGSASIHTTPRequest performSelector:selector onTarget:indicator withObject:nil amount:&progressAmount callerToRetain:nil];
	[progressLock unlock];
}
...
{
#if DEBUG_REQUEST_STATUS || DEBUG_THROTTLING
	MRGSASI_DEBUG_LOG(@"[STATUS] Request finished: %@",self);
#endif
	if ([self error] || [self mainRequest]) {
		return;
	}
...
{
#if DEBUG_REQUEST_STATUS || DEBUG_THROTTLING
	MRGSASI_DEBUG_LOG(@"[STATUS] Request %@: %@",self,(theError == MRGSASIRequestCancelledError ? @"Cancelled" : @"Failed"));
#endif
	[self setComplete:YES];
	
	// Invalidate the current connection so subsequent requests don't attempt to reuse it
	if (theError && [theError code] != MRGSASIAuthenticationErrorType && [theError code] != MRGSASITooMuchRedirectionErrorType) {
		[connectionsLock lock];
		#if DEBUG_PERSISTENT_CONNECTIONS
		MRGSASI_DEBUG_LOG(@"[CONNECTION] Request #%@ failed and will invalidate connection #%@",[self requestID],[[self connectionInfo] objectForKey:@"id"]);
		#endif
	...
	// If we have cached data, use it and ignore the error when using MRGSASIFallbackToCacheIfLoadFailsCachePolicy
	if ([self downloadCache] && ([self cachePolicy] & MRGSASIFallbackToCacheIfLoadFailsCachePolicy)) {
		if ([[self downloadCache] canUseCachedDataForRequest:self]) {
			[self useDataFromCache];
			return;
		}
	...
	MRGSASIHTTPRequest *failedRequest = self;
	
	// If this is a HEAD request created by an MRGSASINetworkQueue or compatible queue delegate, make the main request fail
	if ([self mainRequest]) {
...
- (void)readResponseHeaders
{
	...
	#if DEBUG_REQUEST_STATUS
	if ([self totalBytesSent] == [self postLength]) {
		MRGSASI_DEBUG_LOG(@"[STATUS] Request %@ received response headers",self);
	}
	#endif		
	...
	if ([self responseStatusCode] == 401) {
		[self setAuthenticationNeeded:MRGSASIHTTPAuthenticationNeeded];
	} else if ([self responseStatusCode] == 407) {
		[self setAuthenticationNeeded:MRGSASIProxyAuthenticationNeeded];
	} else {
		#if DEBUG_HTTP_AUTHENTICATION
		if ([self authenticationScheme]) {
			MRGSASI_DEBUG_LOG(@"[AUTH] Request %@ has passed %@ authentication",self,[self authenticationScheme]);
		}
		#endif
	}
	...
	if (![self authenticationNeeded]) {
		...
		if (!requestAuthentication && [[self authenticationScheme] isEqualToString:(NSString *)kCFHTTPAuthenticationSchemeBasic] && [self username] && [self password] && [self useSessionPersistence]) {
			...
			#if DEBUG_HTTP_AUTHENTICATION
			MRGSASI_DEBUG_LOG(@"[AUTH] Request %@ passed BMRGSASIC authentication, and will save credentials in the session store for future use",self);
			#endif
	...
	if ([self useCookiePersistence]) {
		...
		for (cookie in newCookies) {
			[MRGSASIHTTPRequest addSessionCookie:cookie];
		}
	}
	...
	if (![self willRedirect]) {
		...
		MRGSASIHTTPRequest *theRequest = self;
		if ([self mainRequest]) {
			...
			if (![connectionHeader isEqualToString:@"close"]) {
				...
				if (keepAliveHeader) { 
					...
					if (max > 5) {
						...
						#if DEBUG_PERSISTENT_CONNECTIONS
							MRGSASI_DEBUG_LOG(@"[CONNECTION] Got a keep-alive header, will keep this connection open for %f seconds", [self persistentConnectionTimeoutSeconds]);
						#endif					
					}
				...
				// Otherwise, we'll assume we can keep this connection open
					...
					[self setConnectionCanBeReused:YES];
					#if DEBUG_PERSISTENT_CONNECTIONS
						MRGSASI_DEBUG_LOG(@"[CONNECTION] Got no keep-alive header, will keep this connection open for %f seconds", [self persistentConnectionTimeoutSeconds]);
					#endif
				}
			}
		}
...
- (BOOL)willRedirect
{
	...
	// Note that MRGSASIHTTPRequest does not currently support 305 Use Proxy
	...
	#if DEBUG_REQUEST_STATUS
	MRGSASI_DEBUG_LOG(@"[STATUS] Request will redirect (code: %i): %@",responseCode,self);
	#endif
...
- (void)saveProxyCredentialsToKeychain:(NSDictionary *)newCredentials
{
	...
	if (authenticationCredentials) {
		[MRGSASIHTTPRequest saveCredentials:authenticationCredentials forProxy:[self proxyHost] port:[self proxyPort] realm:[self proxyAuthenticationRealm]];
	}	
}
...
- (void)saveCredentialsToKeychain:(NSDictionary *)newCredentials
{
	...
	if (authenticationCredentials) {
		[MRGSASIHTTPRequest saveCredentials:authenticationCredentials forHost:[[self url] host] port:[[[self url] port] intValue] protocol:[[self url] scheme] realm:[self authenticationRealm]];
	}	
}
...
- (NSMutableDictionary *)findProxyCredentials
{
	...
	MRGSASIHTTPRequest *theRequest = [self mainRequest];
	// If this is a HEAD request generated by an MRGSASINetworkQueue, we'll try to use the details from the main request
	...
	if ((!user || !pass)) {
		NSURLCredential *authenticationCredentials = [MRGSASIHTTPRequest savedCredentialsForProxy:[self proxyHost] port:[self proxyPort] protocol:[[self url] scheme] realm:[self proxyAuthenticationRealm]];
		if (authenticationCredentials) {
...
- (NSMutableDictionary *)findCredentials
{
	...
	if (user && pass) {
		...
		#if DEBUG_HTTP_AUTHENTICATION
		MRGSASI_DEBUG_LOG(@"[AUTH] Request %@ will use credentials set on its url",self);
		#endif
		...
		if ([self mainRequest] && [[self mainRequest] username] && [[self mainRequest] password]) {
			...
			#if DEBUG_HTTP_AUTHENTICATION
			MRGSASI_DEBUG_LOG(@"[AUTH] Request %@ will use credentials from its parent request",self);
			#endif
		...
		// Let's try to use the ones set in this object
			...
			#if DEBUG_HTTP_AUTHENTICATION
			MRGSASI_DEBUG_LOG(@"[AUTH] Request %@ will use username and password properties as credentials",self);
			#endif
		}		
	}
	...
	if ((!user || !pass) && useKeychainPersistence) {
		NSURLCredential *authenticationCredentials = [MRGSASIHTTPRequest savedCredentialsForHost:[[self url] host] port:[[[self url] port] intValue] protocol:[[self url] scheme] realm:[self authenticationRealm]];
		if (authenticationCredentials) {
			user = [authenticationCredentials user];
			pass = [authenticationCredentials password];
			#if DEBUG_HTTP_AUTHENTICATION
			if (user && pass) {
				MRGSASI_DEBUG_LOG(@"[AUTH] Request %@ will use credentials from the keychain",self);
			}
			#endif
		}
	}
...
{
	#if DEBUG_HTTP_AUTHENTICATION
		MRGSASI_DEBUG_LOG(@"[AUTH] Request %@ received credentials from its delegate or an MRGSASIAuthenticationDialog, will retry",self);
	#endif
...
{
	#if DEBUG_HTTP_AUTHENTICATION
		MRGSASI_DEBUG_LOG(@"[AUTH] Request %@ had authentication cancelled by its delegate or an MRGSASIAuthenticationDialog",self);
	#endif
...
- (void)failAuthentication
{
	[self failWithError:MRGSASIAuthenticationError];
}
...
- (void)attemptToApplyProxyCredentialsAndResume
{
	...
	if (!proxyAuthentication) {
		[self cancelLoad];
		[self failWithError:[NSError errorWithDomain:NetworkRequestErrorDomain code:MRGSASIInternalErrorWhileApplyingCredentialsType userInfo:[NSDictionary dictionaryWithObjectsAndKeys:@"Failed to get authentication object from response headers",NSLocalizedDescriptionKey,nil]]];
		return;
	}
	...
	if (proxyCredentials) {
		...
		// Something went wrong, we'll have to give up
			...
			[self failWithError:[NSError errorWithDomain:NetworkRequestErrorDomain code:MRGSASIInternalErrorWhileApplyingCredentialsType userInfo:[NSDictionary dictionaryWithObjectsAndKeys:@"Failed to apply proxy credentials to request",NSLocalizedDescriptionKey,nil]]];
		}
...
- (void)attemptToApplyCredentialsAndResume
{
	...
	if ([self authenticationNeeded] == MRGSASIProxyAuthenticationNeeded) {
		[self attemptToApplyProxyCredentialsAndResume];
		return;
	}
	...
	if (!requestAuthentication) {
		#if DEBUG_HTTP_AUTHENTICATION
		MRGSASI_DEBUG_LOG(@"[AUTH] Request %@ failed to read authentication information from response headers",self);
		#endif
	...
	if (!CFHTTPAuthenticationRequiresAccountDomain(requestAuthentication)) {
		...
		if ([self authenticationScheme] != (NSString *)kCFHTTPAuthenticationSchemeNTLM || [self authenticationRetryCount] == 0) {
			MRGSASI_DEBUG_LOG(@"[AUTH] Request %@ received 401 challenge and must authenticate using %@%@",self,[self authenticationScheme],realm);
		} else {
			MRGSASI_DEBUG_LOG(@"[AUTH] Request %@ NTLM handshake step %i",self,[self authenticationRetryCount]+1);
		}
	#endif
	...
	if (!CFHTTPAuthenticationIsValid(requestAuthentication, &err)) {
		...
		if (err.domain == kCFStreamErrorDomainHTTP && (err.error == kCFStreamErrorHTTPAuthenticationBadUserName || err.error == kCFStreamErrorHTTPAuthenticationBadPassword)) {
			...
			#if DEBUG_HTTP_AUTHENTICATION
			MRGSASI_DEBUG_LOG(@"[AUTH] Request %@ had bad credentials, will remove them from the session store if they are cached",self);
			#endif
			...
			if ([self error] || [self isCancelled]) {
				...
				#if DEBUG_HTTP_AUTHENTICATION
				MRGSASI_DEBUG_LOG(@"[AUTH] Request %@ failed or was cancelled while waiting to access credentials",self);
				#endif
			...
			if ([self useSessionPersistence]) {
				...
				if (credentials && [self applyCredentials:[credentials objectForKey:@"Credentials"]]) {
					...
					#if DEBUG_HTTP_AUTHENTICATION
					MRGSASI_DEBUG_LOG(@"[AUTH] Request %@ will reuse cached credentials from the session (%@)",self,[credentials objectForKey:@"AuthenticationScheme"]);
					#endif
			...
			if ([self willAskDelegateForCredentials]) {
				...
				#if DEBUG_HTTP_AUTHENTICATION
				MRGSASI_DEBUG_LOG(@"[AUTH] Request %@ will ask its delegate for credentials to use",self);
				#endif
			...
			if ([self showAuthenticationDialog]) {
				...
				#if DEBUG_HTTP_AUTHENTICATION
				MRGSASI_DEBUG_LOG(@"[AUTH] Request %@ will ask MRGSASIAuthenticationDialog for credentials",self);
				#endif
		...
		#if DEBUG_HTTP_AUTHENTICATION
		MRGSASI_DEBUG_LOG(@"[AUTH] Request %@ has no credentials to present and must give up",self);
		#endif
	...
	if (requestCredentials) {
		...
		if ((([self authenticationScheme] != (NSString *)kCFHTTPAuthenticationSchemeNTLM) || [self authenticationRetryCount] < 2) && [self applyCredentials:requestCredentials]) {
			...
			#if DEBUG_HTTP_AUTHENTICATION
			MRGSASI_DEBUG_LOG(@"[AUTH] Request %@ has failed NTLM authentication",self);
			#endif
			...
			#if DEBUG_HTTP_AUTHENTICATION
			MRGSASI_DEBUG_LOG(@"[AUTH] Request %@ had credentials and they were not marked as bad, but we got a 401 all the same.",self);
			#endif

			[self failWithError:[NSError errorWithDomain:NetworkRequestErrorDomain code:MRGSASIInternalErrorWhileApplyingCredentialsType userInfo:[NSDictionary dictionaryWithObjectsAndKeys:@"Failed to apply credentials to request",NSLocalizedDescriptionKey,nil]]];
		}
		...
		if (newCredentials) {
			...
			if ([self applyCredentials:newCredentials]) {
				...
				#if DEBUG_HTTP_AUTHENTICATION
				MRGSASI_DEBUG_LOG(@"[AUTH] Request %@ failed to apply credentials",self);
				#endif
				[delegateAuthenticationLock unlock];
				[self failWithError:[NSError errorWithDomain:NetworkRequestErrorDomain code:MRGSASIInternalErrorWhileApplyingCredentialsType userInfo:[NSDictionary dictionaryWithObjectsAndKeys:@"Failed to apply credentials to request",NSLocalizedDescriptionKey,nil]]];
			}
...
{
	[self addRequestHeader:@"Authorization" value:[NSString stringWithFormat:@"Basic %@",[MRGSASIHTTPRequest base64forData:[[NSString stringWithFormat:@"%@:%@",theUsername,thePassword] dataUsingEncoding:NSUTF8StringEncoding]]]];	
	...
	// If zero bytes were read, wait for the EOF to come.
		...
		if ([self isResponseCompressed] && ![self shouldWaitToInflateCompressedResponses]) {
			if (![self dataDecompressor]) {
				[self setDataDecompressor:[MRGSASIDataDecompressor decompressor]];
			}
			NSError *err = nil;
		...
		[MRGSASIHTTPRequest incrementBandwidthUsedInLastSecond:bytesRead];
...
{	
...
#if DEBUG_REQUEST_STATUS
	MRGSASI_DEBUG_LOG(@"[STATUS] Request %@ finished downloading data (%qu bytes)",self, [self totalBytesRead]);
#endif
	[self setStatusTimer:nil];
	...
	if ([self temporaryFileDownloadPath]) {
		...
		if ([self shouldRedirect] && [self needsRedirect] && [self allowResumeForFileDownloads]) {
			...
			if ([self shouldWaitToInflateCompressedResponses]) {
				[MRGSASIDataDecompressor uncompressDataFromFile:[self temporaryFileDownloadPath] toFile:[self downloadDestinationPath] error:&fileError];
	...
	#if DEBUG_PERSISTENT_CONNECTIONS
	if ([self requestID]) {
		MRGSASI_DEBUG_LOG(@"[CONNECTION] Request #%@ finished using connection #%@",[self requestID], [[self connectionInfo] objectForKey:@"id"]);
	}
	#endif
	[[self connectionInfo] removeObjectForKey:@"request"];
	[[self connectionInfo] setObject:[NSDate dateWithTimeIntervalSinceNow:[self persistentConnectionTimeoutSeconds]] forKey:@"expires"];
	...
	// If request has asked delegate or MRGSASIAuthenticationDialog for credentials
...
- (void)useDataFromCache
{
	...
	if (headers && dataPath) {
		...
		[self setResponseStatusCode:[[headers objectForKey:@"X-MRGSASIHTTPRequest-Response-Status-Code"] intValue]];
...
- (BOOL)retryUsingNewConnection
{
	if ([self retryCount] == 0) {
		...
		#if DEBUG_PERSISTENT_CONNECTIONS
			MRGSASI_DEBUG_LOG(@"[CONNECTION] Request attempted to use connection #%@, but it has been closed - will retry with a new connection", [[self connectionInfo] objectForKey:@"id"]);
		#endif
		[connectionsLock lock];
		[[self connectionInfo] removeObjectForKey:@"request"];
		[persistentConnectionsPool removeObject:[self connectionInfo]];
	...
	#if DEBUG_PERSISTENT_CONNECTIONS
		MRGSASI_DEBUG_LOG(@"[CONNECTION] Request attempted to use connection #%@, but it has been closed - we have already retried with a new connection, so we must give up", [[self connectionInfo] objectForKey:@"id"]);
	#endif	
	return NO;
}
...

{
	...
	if (![self error]) { // We may already have handled this error
		...
		[self cancelLoad];
		[self failWithError:[NSError errorWithDomain:NetworkRequestErrorDomain code:MRGSASIConnectionFailureErrorType userInfo:[NSDictionary dictionaryWithObjectsAndKeys:reason,NSLocalizedDescriptionKey,underlyingError,NSUnderlyingErrorKey,nil]]];
	} else {
		[self cancelLoad];
	}
	[self checkRequestStatus];
...
+ (BOOL)removeFileAtPath:(NSString *)path error:(NSError **)err
{
	...
	if ([fileManager fileExistsAtPath:path]) {
		...
		if (removeError) {
			if (err) {
				*err = [NSError errorWithDomain:NetworkRequestErrorDomain code:MRGSASIFileManagementError userInfo:[NSDictionary dictionaryWithObjectsAndKeys:[NSString stringWithFormat:@"Failed to delete file at path '%@'",path],NSLocalizedDescriptionKey,removeError,NSUnderlyingErrorKey,nil]];
			}
			return NO;
		}
	}
...
- (BOOL)configureProxies
{
	...
	if (![self isPACFileRequest] && (![self proxyHost] && ![self proxyPort])) {
		...
		if (!proxies) {
			[self setReadStream:nil];
			[self failWithError:[NSError errorWithDomain:NetworkRequestErrorDomain code:MRGSASIInternalErrorWhileBuildingRequestType userInfo:[NSDictionary dictionaryWithObjectsAndKeys:@"Unable to obtain information on proxy servers needed for request",NSLocalizedDescriptionKey,nil]]];
			return NO;
		}
...
- (void)fetchPACFile
{
	// For file:// urls, we'll use an async NSInputStream (MRGSASIHTTPRequest does not support file:// urls)
	...
	// Create an MRGSASIHTTPRequest to fetch the PAC file
	MRGSASIHTTPRequest *PACRequest = [MRGSASIHTTPRequest requestWithURL:[self PACurl]];
...
#if DEBUG_PERSISTENT_CONNECTIONS
			MRGSASI_DEBUG_LOG(@"[CONNECTION] Closing connection #%i because it has expired",[[existingConnection objectForKey:@"id"] intValue]);
#endif
...
- (id)copyWithZone:(NSZone *)zone
{
	...
	MRGSASIHTTPRequest *newRequest = [[[self class] alloc] initWithURL:[self url]];
...
+ (NSMutableArray *)sessionCookies
{
	...
	if (!sessionCookies) {
		[MRGSASIHTTPRequest setSessionCookies:[NSMutableArray array]];
	}
...
+ (void)addSessionCookie:(NSHTTPCookie *)newCookie
{
	...
	NSHTTPCookie *cookie;
	NSUInteger i;
	NSUInteger max = [[MRGSASIHTTPRequest sessionCookies] count];
	for (i=0; i<max; i++) {
		cookie = [[MRGSASIHTTPRequest sessionCookies] objectAtIndex:i];
		if ([[cookie domain] isEqualToString:[newCookie domain]] && [[cookie path] isEqualToString:[newCookie path]] && [[cookie name] isEqualToString:[newCookie name]]) {
			[[MRGSASIHTTPRequest sessionCookies] removeObjectAtIndex:i];
			break;
		}
	}
	[[MRGSASIHTTPRequest sessionCookies] addObject:newCookie];
	[sessionCookiesLock unlock];
}
...
+ (void)clearSession
{
	...
	[[[self class] defaultCache] clearCachedResponsesForStoragePolicy:MRGSASICacheForSessionDurationCacheStoragePolicy];
}
...
+ (NSString *)defaultUserAgentString
{
	@synchronized (self) {
		...
		if (!defaultUserAgent) {
			...
			// If we couldn't find one, we'll give up (and MRGSASIHTTPRequest will use the standard CFNetwork user agent)
			if (!appName) {
				return nil;
			}
			...
			NSString *locale = [[NSLocale currentLocale] localeIdentifier];
				...
				// We won't bother to check for systems prior to 10.4, since MRGSASIHTTPRequest only works on 10.5+
				OSErr err;
				...
				#else
					OSVersion = [NSString stringWithFormat:@"%lu.%lu.%lu", versionMajor, versionMinor, versionBugFix];
				#endif
			#endif
...
- (void)performThrottling
{
	...
	[MRGSASIHTTPRequest measureBandwidthUsage];
	if ([MRGSASIHTTPRequest isBandwidthThrottled]) {
		[bandwidthThrottlingLock lock];
		// Handle throttling
		if (throttleWakeUpTime) {
			if ([throttleWakeUpTime timeIntervalSinceDate:[NSDate date]] > 0) {
				if ([self readStreamIsScheduled]) {
					[self unscheduleReadStream];
					#if DEBUG_THROTTLING
					MRGSASI_DEBUG_LOG(@"[THROTTLING] Sleeping request %@ until after %@",self,throttleWakeUpTime);
					#endif
				}
			} else {
				if (![self readStreamIsScheduled]) {
					[self scheduleReadStream];
					#if DEBUG_THROTTLING
					MRGSASI_DEBUG_LOG(@"[THROTTLING] Waking up request %@",self);
					#endif
				}
			}
		} 
...
+ (void)recordBandwidthUsage
{
	...
	#if DEBUG_THROTTLING
	MRGSASI_DEBUG_LOG(@"[THROTTLING] ===Used: %u bytes of bandwidth in last measurement period===",bandwidthUsedInLastSecond);
	#endif
...
+ (void)measureBandwidthUsage
{
	...
	if (!bandwidthMeasurementDate || [bandwidthMeasurementDate timeIntervalSinceNow] < -0) {
		[MRGSASIHTTPRequest recordBandwidthUsage];
	}
...
+ (unsigned long)maxUploadReadLength
{
	...
	// We'll split our bandwidth allowance into 4 (which is the default for an MRGSASINetworkQueue's max concurrent operations count) to give all running requests a fighting chance of reading data this cycle
...
{
	if (throttle) {
		[MRGSASIHTTPRequest throttleBandwidthForWWANUsingLimit:MRGSASIWWANBandwidthThrottleAmount];
	} else {
		[MRGSASIHTTPRequest unsubscribeFromNetworkReachabilityNotifications];
		[MRGSASIHTTPRequest setMaxBandwidthPerSecond:0];
		[bandwidthThrottlingLock lock];
...
{	
	...
	[MRGSASIHTTPRequest registerForNetworkReachabilityNotifications];	
	[bandwidthThrottlingLock unlock];
	[MRGSASIHTTPRequest reachabilityChanged:nil];
}
...
+ (void)registerForNetworkReachabilityNotifications
{
	[[MRGSReachability reachabilityForInternetConnection] startNotifier];
	[[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(reachabilityChanged:) name:kMRGSReachabilityChangedNotification object:nil];
}


+ (void)unsubscribeFromNetworkReachabilityNotifications
{
	[[NSNotificationCenter defaultCenter] removeObserver:self name:kMRGSReachabilityChangedNotification object:nil];
}

+ (BOOL)isNetworkReachableViaWWAN
{
	return ([[MRGSReachability reachabilityForInternetConnection] currentReachabilityStatus] == MRGSReachableViaWWAN);	
}

+ (void)reachabilityChanged:(NSNotification *)note
{
	[bandwidthThrottlingLock lock];
	isBandwidthThrottled = [MRGSASIHTTPRequest isNetworkReachableViaWWAN];
	[bandwidthThrottlingLock unlock];
}
#endif
...
+ (void)setDefaultCache:(id <MRGSASICacheDelegate>)cache
{
	@synchronized (self) {
		[cache retain];
...
+ (id <MRGSASICacheDelegate>)defaultCache
{
    @synchronized(self) {
...
+ (NSThread *)threadForRequest:(MRGSASIHTTPRequest *)request
{
...
+ (NSDate *)expiryDateForRequest:(MRGSASIHTTPRequest *)request maxAge:(NSTimeInterval)maxAge
{
	...
	if (maxAge) {
		...
		if (expires) {
			return [MRGSASIHTTPRequest dateFromRFC1123String:expires];
		}
	}
	return nil;
}
...
#pragma mark blocks
#if NS_BLOCKS_AVAILABLE
- (void)setStartedBlock:(MRGSASIBasicBlock)aStartedBlock
{
	[startedBlock release];
	startedBlock = [aStartedBlock copy];
}

- (void)setHeadersReceivedBlock:(MRGSASIHeadersBlock)aReceivedBlock
{
	[headersReceivedBlock release];
	headersReceivedBlock = [aReceivedBlock copy];
}

- (void)setCompletionBlock:(MRGSASIBasicBlock)aCompletionBlock
{
	[completionBlock release];
	completionBlock = [aCompletionBlock copy];
}

- (void)setFailedBlock:(MRGSASIBasicBlock)aFailedBlock
{
	[failureBlock release];
	failureBlock = [aFailedBlock copy];
}

- (void)setBytesReceivedBlock:(MRGSASIProgressBlock)aBytesReceivedBlock
{
	[bytesReceivedBlock release];
	bytesReceivedBlock = [aBytesReceivedBlock copy];
}

- (void)setBytesSentBlock:(MRGSASIProgressBlock)aBytesSentBlock
{
	[bytesSentBlock release];
	bytesSentBlock = [aBytesSentBlock copy];
}

- (void)setDownloadSizeIncrementedBlock:(MRGSASISizeBlock)aDownloadSizeIncrementedBlock{
	[downloadSizeIncrementedBlock release];
	downloadSizeIncrementedBlock = [aDownloadSizeIncrementedBlock copy];
}

- (void)setUploadSizeIncrementedBlock:(MRGSASISizeBlock)anUploadSizeIncrementedBlock
{
	[uploadSizeIncrementedBlock release];
	uploadSizeIncrementedBlock = [anUploadSizeIncrementedBlock copy];
}

- (void)setDataReceivedBlock:(MRGSASIDataBlock)aReceivedBlock
{
	[dataReceivedBlock release];
	dataReceivedBlock = [aReceivedBlock copy];
}

- (void)setAuthenticationNeededBlock:(MRGSASIBasicBlock)anAuthenticationBlock
{
	[authenticationNeededBlock release];
	authenticationNeededBlock = [anAuthenticationBlock copy];
}
- (void)setProxyAuthenticationNeededBlock:(MRGSASIBasicBlock)aProxyAuthenticationBlock
{
	[proxyAuthenticationNeededBlock release];
	proxyAuthenticationNeededBlock = [aProxyAuthenticationBlock copy];
}
- (void)setRequestRedirectedBlock:(MRGSASIBasicBlock)aRedirectBlock
{
	[requestRedirectedBlock release];
