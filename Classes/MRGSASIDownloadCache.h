...
#import "MRGSASICacheDelegate.h"

@interface MRGSASIDownloadCache : NSObject <MRGSASICacheDelegate> {
	
	// The default cache policy for this cache
	// Requests that store data in the cache will use this cache policy if their cache policy is set to MRGSASIUseDefaultCachePolicy
	// Defaults to MRGSASIAskServerIfModifiedWhenStaleCachePolicy
	MRGSASICachePolicy defaultCachePolicy;
	
	// The directory in which cached data will be stored
	// Defaults to a directory called 'MRGSASIHTTPRequestCache' in the temporary directory
...
// Returns a static instance of an MRGSASIDownloadCache
// In most circumstances, it will make sense to use this as a global cache, rather than creating your own cache
// To make MRGSASIHTTPRequests use it automatically, use [MRGSASIHTTPRequest setDefaultCache:[MRGSASIDownloadCache sharedCache]];
+ (id)sharedCache;
...
@property (assign, nonatomic) MRGSASICachePolicy defaultCachePolicy;
