...
#import "MRGSASIDownloadCache.h"
#import "MRGSASIHTTPRequest.h"
#import <CommonCrypto/CommonHMAC.h>

static MRGSASIDownloadCache *sharedCache = nil;

static NSString *sessionCacheFolder = @"SessionStore";
static NSString *permanentCacheFolder = @"PermanentStore";
static NSArray *fileExtensionsToHandleAsHTML = nil;

@interface MRGSASIDownloadCache ()
+ (NSString *)keyForURL:(NSURL *)url;
- (NSString *)pathToFile:(NSString *)file;
@end

@implementation MRGSASIDownloadCache

+ (void)initialize
{
	if (self == [MRGSASIDownloadCache class]) {
...
- (id)init
{
	...
	[self setDefaultCachePolicy:MRGSASIUseDefaultCachePolicy];
	...
	if (!sharedCache) {
		@synchronized(self) {
			if (!sharedCache) {
				...
				[sharedCache setStoragePath:[[NSSearchPathForDirectoriesInDomains(NSCachesDirectory, NSUserDomainMask, YES) objectAtIndex:0] stringByAppendingPathComponent:@"MRGSASIHTTPRequestCache"]];
			}
		}
	}
	return sharedCache;
...
{
	[[self accessLock] lock];
	[self clearCachedResponsesForStoragePolicy:MRGSASICacheForSessionDurationCacheStoragePolicy];
	[storagePath release];
...
- (void)updateExpiryForRequest:(MRGSASIHTTPRequest *)request maxAge:(NSTimeInterval)maxAge
{
	...
	[cachedHeaders setObject:[NSNumber numberWithDouble:[expires timeIntervalSince1970]] forKey:@"X-MRGSASIHTTPRequest-Expires"];
...
{
  return [MRGSASIHTTPRequest expiryDateForRequest:request maxAge:maxAge];
}
...
- (void)storeResponseForRequest:(MRGSASIHTTPRequest *)request maxAge:(NSTimeInterval)maxAge
{
	...
	if ([request error] || ![request responseHeaders] || ([request cachePolicy] & MRGSASIDoNotWriteToCacheCachePolicy)) {
		[[self accessLock] unlock];
		return;
	}
	...
	// Create a special 'X-MRGSASIHTTPRequest-Expires' header
	...
	NSDate *expires = [self expiryDateForRequest:request maxAge:maxAge];
	if (expires) {
		[responseHeaders setObject:[NSNumber numberWithDouble:[expires timeIntervalSince1970]] forKey:@"X-MRGSASIHTTPRequest-Expires"];
	}
	...
	[responseHeaders setObject:[NSNumber numberWithInt:statusCode] forKey:@"X-MRGSASIHTTPRequest-Response-Status-Code"];
...
- (NSString *)pathToStoreCachedResponseDataForRequest:(MRGSASIHTTPRequest *)request
{
	...
	NSString *path = [[self storagePath] stringByAppendingPathComponent:([request cacheStoragePolicy] == MRGSASICacheForSessionDurationCacheStoragePolicy ? sessionCacheFolder : permanentCacheFolder)];
...
- (BOOL)isCachedDataCurrentForRequest:(MRGSASIHTTPRequest *)request
{
	...
	if ([self shouldRespectCacheControlHeaders]) {
		...
		// Look for X-MRGSASIHTTPRequest-Expires header to see if the content is out of date
		NSNumber *expires = [cachedHeaders objectForKey:@"X-MRGSASIHTTPRequest-Expires"];
		if (expires) {
...
{
	[[self accessLock] lock];
	MRGSASICachePolicy cp = defaultCachePolicy;
	[[self accessLock] unlock];
	return cp;
}


- (void)setDefaultCachePolicy:(MRGSASICachePolicy)cachePolicy
{
	[[self accessLock] lock];
	if (!cachePolicy) {
		defaultCachePolicy = MRGSASIAskServerIfModifiedWhenStaleCachePolicy;
	}  else {
...
- (void)clearCachedResponsesForStoragePolicy:(MRGSASICacheStoragePolicy)storagePolicy
{
	[[self accessLock] lock];
	if (![self storagePath]) {
		[[self accessLock] unlock];
	...
	NSString *path = [[self storagePath] stringByAppendingPathComponent:(storagePolicy == MRGSASICacheForSessionDurationCacheStoragePolicy ? sessionCacheFolder : permanentCacheFolder)];
...
+ (BOOL)serverAllowsResponseCachingForRequest:(MRGSASIHTTPRequest *)request
{
...
+ (NSString *)keyForURL:(NSURL *)url
{
	...
	// Strip trailing slashes so http://allseeing-i.com/MRGSASIHTTPRequest/ is cached the same as http://allseeing-i.com/MRGSASIHTTPRequest
...
- (BOOL)canUseCachedDataForRequest:(MRGSASIHTTPRequest *)request
{
	...
	if ([request cachePolicy] & MRGSASIDoNotReadFromCacheCachePolicy) {
		return NO;

	// If we don't want to load the request whatever happens, always pretend we have cached data even if we don't
	} else if ([request cachePolicy] & MRGSASIDontLoadCachePolicy) {
		return YES;
	}
	...
	if ([request cachePolicy] & MRGSASIOnlyLoadIfNotCachedCachePolicy) {
		return YES;

	// If we want to fallback to the cache after an error
	} else if ([request complete] && [request cachePolicy] & MRGSASIFallbackToCacheIfLoadFailsCachePolicy) {
		return YES;

	// If we have cached data that is current, we can use it
	} else if ([request cachePolicy] & MRGSASIAskServerIfModifiedWhenStaleCachePolicy) {
	...
	} else if ([request cachePolicy] & MRGSASIAskServerIfModifiedCachePolicy) {
