...
@implementation MRGSASIInputStream

+ (void)initialize
{
	if (self == [MRGSASIInputStream class]) {
		readLock = [[NSLock alloc] init];
	}
}

+ (id)inputStreamWithFileAtPath:(NSString *)path request:(MRGSASIHTTPRequest *)theRequest
{
	MRGSASIInputStream *theStream = [[[self alloc] init] autorelease];
	[theStream setRequest:theRequest];
	[theStream setStream:[NSInputStream inputStreamWithFileAtPath:path]];
	return theStream;
}

+ (id)inputStreamWithData:(NSData *)data request:(MRGSASIHTTPRequest *)theRequest
{
	MRGSASIInputStream *theStream = [[[self alloc] init] autorelease];
	[theStream setRequest:theRequest];
...
// When throttling is on, we ask MRGSASIHTTPRequest for the maximum amount of data we can read
- (NSInteger)read:(uint8_t *)buffer maxLength:(NSUInteger)len
{
	[readLock lock];
	unsigned long toRead = len;
	if ([MRGSASIHTTPRequest isBandwidthThrottled]) {
		toRead = [MRGSASIHTTPRequest maxUploadReadLength];
		if (toRead > len) {
			toRead = len;
		} else if (toRead == 0) {
			toRead = 1;
	...
	if (rv > 0)
		[MRGSASIHTTPRequest incrementBandwidthUsedInLastSecond:rv];
	return rv;
}
