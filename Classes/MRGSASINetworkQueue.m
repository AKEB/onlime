...
- (void)dealloc
{
	...
	for (MRGSASIHTTPRequest *request in [self operations]) {
...
- (void)resetProgressDelegate:(id *)progressDelegate
{
	...
	if ([*progressDelegate respondsToSelector:selector]) {
		double max = 1.0;
		[MRGSASIHTTPRequest performSelector:selector onTarget:progressDelegate withObject:nil amount:&max callerToRetain:nil];
	}
	selector = @selector(setDoubleValue:);
	if ([*progressDelegate respondsToSelector:selector]) {
		double value = 0.0;
		[MRGSASIHTTPRequest performSelector:selector onTarget:progressDelegate withObject:nil amount:&value callerToRetain:nil];
	}
#else
...
{
	if ([operation isKindOfClass:[MRGSASIHTTPRequest class]]) {
		
		MRGSASIHTTPRequest *request = (MRGSASIHTTPRequest *)operation;
...
// Only add MRGSASIHTTPRequests to this queue!!
- (void)addOperation:(NSOperation *)operation
{
	if (![operation isKindOfClass:[MRGSASIHTTPRequest class]]) {
		[NSException raise:@"AttemptToAddInvalidRequest" format:@"Attempted to add an object that was not an MRGSASIHTTPRequest to an MRGSASINetworkQueue"];
	}
	...
	if ([self showAccurateProgress]) {
		...
		if ([[request requestMethod] isEqualToString:@"GET"]) {
			if ([self isSuspended]) {
				MRGSASIHTTPRequest *HEADRequest = [request HEADRequest];
...
- (void)request:(MRGSASIHTTPRequest *)request didReceiveBytes:(long long)bytes
{
	[self setBytesDownloadedSoFar:[self bytesDownloadedSoFar]+bytes];
	if ([self downloadProgressDelegate]) {
		[MRGSASIHTTPRequest updateProgressIndicator:&downloadProgressDelegate withProgress:[self bytesDownloadedSoFar] ofTotal:[self totalBytesToDownload]];
	}
}

- (void)request:(MRGSASIHTTPRequest *)request didSendBytes:(long long)bytes
{
	[self setBytesUploadedSoFar:[self bytesUploadedSoFar]+bytes];
	if ([self uploadProgressDelegate]) {
		[MRGSASIHTTPRequest updateProgressIndicator:&uploadProgressDelegate withProgress:[self bytesUploadedSoFar] ofTotal:[self totalBytesToUpload]];
	}
}

- (void)request:(MRGSASIHTTPRequest *)request incrementDownloadSizeBy:(long long)newLength
{
	[self setTotalBytesToDownload:[self totalBytesToDownload]+newLength];
}

- (void)request:(MRGSASIHTTPRequest *)request incrementUploadSizeBy:(long long)newLength
{
...
{
	MRGSASINetworkQueue *newQueue = [[[self class] alloc] init];
