...
// Private stuff
@interface MRGSASIFormDataRequest ()
...
@implementation MRGSASIFormDataRequest
...
- (id)initWithURL:(NSURL *)newURL
{
	...
	[self setPostFormat:MRGSASIURLEncodedPostFormat];
...
- (void)addFile:(NSString *)filePath withFileName:(NSString *)fileName andContentType:(NSString *)contentType forKey:(NSString *)key
{
	...
	if (!fileExists || isDirectory) {
		[self failWithError:[NSError errorWithDomain:NetworkRequestErrorDomain code:MRGSASIInternalErrorWhileBuildingRequestType userInfo:[NSDictionary dictionaryWithObjectsAndKeys:[NSString stringWithFormat:@"No file exists at %@",filePath],NSLocalizedDescriptionKey,nil]]];
	}
	...
	if (!contentType) {
		contentType = [MRGSASIHTTPRequest mimeTypeForFileAtPath:filePath];
	}
...
- (void)buildPostBody
{
	...
	if ([self postFormat] == MRGSASIURLEncodedPostFormat) {
...
#if DEBUG_FORM_DATA_REQUEST
	MRGSASI_DEBUG_LOG(@"%@",[self debugBodyString]);
...
- (void)buildURLEncodedPostBody
{
	...
	if ([[self fileData] count] > 0) {
		[self setPostFormat:MRGSASIMultipartFormDataPostFormat];
		[self buildMultipartFormDataPostBody];
		return;
	}
...
{
	MRGSASIFormDataRequest *newRequest = [super copyWithZone:zone];
